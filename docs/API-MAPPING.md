...
| `Workload` | `GET /api/v1/backupObjects` | `id`, `name`, `type`, `platformName`, `platformId`, `restorePointsCount`, `backupId`, `size` |
| `Repo` | `GET /api/v1/backupInfrastructure/repositories` + `/repositories/states` | `type`, `bucket.immutability{isEnabled, daysCount, immutabilityMode}`, `repository.makeRecentBackupsImmutableDays`; states: `capacityGB`, `freeGB`, `usedSpaceGB`, `isOnline` |
...
| `dataSize`, `sizeGB`, `compressRatio`, `dedupRatio`, `file` | **`GET /api/v1/backups/{id}/backupFiles`** → `dataSize`, `backupSize`, `dedupRatio`, `compressRatio`, `name`, `restorePointIds[]`, `objectIds[]`, `severity`. Join por `restorePoint.backupFileId` |
